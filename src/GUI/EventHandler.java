...
public class EventHandler implements ActionListener {
	...
	EventHandler() throws ClassNotFoundException, SQLException {
		...
		settingsPanel.choosePDFTemplateLocation.addActionListener(this);
		settingsPanel.choosePDFOutputLocation.addActionListener(this);
	...
	/**
	 * Create the report manager menu
	 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == editErrorPanel.saveButton) {
			...
			if (editErrorPanel.txtEnfID.getText().length() > 0 && editErrorPanel.txtSapUserName.getText().length() > 0
					&& editErrorPanel.cmbDepartment.getSelectedItem().toString().length() > 0
					&& editErrorPanel.cmbShift.getSelectedItem().toString().length() > 0 
					&& editErrorPanel.model.toString().length() > 0
					...
					&& editErrorPanel.errorDateModel.toString().length() > 0
					&& editErrorPanel.cmbStatus.getSelectedItem().toString().length() > 0
					&& editErrorPanel.txtNotes.getText().length() > 0
					&& editErrorPanel.txtOpenedBy.getText().length() > 0) {
				DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MM/dd/yyyy");
				LocalDate ldt = LocalDate.now();
				DateFormat df = new SimpleDateFormat("MM/dd/yyyy");
				String opened_date = df.format(editErrorPanel.model.getValue());
				String dateOfError = df.format(editErrorPanel.errorDateModel.getValue());

				sql.updateError(editErrorPanel.txtEnfID.getText(), editErrorPanel.txtSapUserName.getText(),
						editErrorPanel.cmbDepartment.getSelectedItem().toString(),
						editErrorPanel.cmbShift.getSelectedItem().toString(),
						opened_date,
						...
						dateOfError,
						editErrorPanel.cmbStatus.getSelectedItem().toString(),
						(ldt.plusDays(7)).format(formatter).toString(),
		...
		if (e.getSource() == editUserPanel.editUserSaveButton) {
			...
			if (editUserPanel.txtSapUserName.getText().length() > 0 && editUserPanel.txtAdminID.getText().length() > 0
					&& editUserPanel.txtJobTitle.getText().length() > 0 && editUserPanel.txtEmail.getText().length() > 0
					&& editUserPanel.cmbAccessLevel.getSelectedItem().toString().length() > 0
					&& editUserPanel.cmbDepartment.getSelectedItem().toString().length() > 0
				...
				if (pw.equals(confirmPw) && pw.length() > 0) {
					...
					sql.updateUser(editUserPanel.txtAdminID.getText(),
							editUserPanel.txtSapUserName.getText(), pw,
		...
		try {
			...
			if (e.getSource() == frame.logoutItem) {
				...
				if (confirm == JOptionPane.YES_OPTION) {
					...
					JOptionPane.showMessageDialog(frame, "Logged Out!", "Logged Out!", JOptionPane.INFORMATION_MESSAGE);
		...
		if (e.getSource() == frame.viewItem) {
			try {
				...
				if (enfID.length() > 0) {
					...
					// set the path of the database to the current path of the
					// program
					// String template = directory + File.separator +
					// "rpt_Error_Notification_template.pdf";
		...
		if (e.getSource() == errorPanel.printButton) {
			...
			String enfID = JOptionPane.showInputDialog("Enter ENF ID Number to print:");
			
			//FileInputStream fis = new FileInputStream(settings.getPdfOutPath() + File.pathSeparator + "enf.pdf");
			PrintHandler print = new PrintHandler();
			PrintService ps = print.choosePrinter();
		...
		if (e.getSource() == addErrorPanel.saveButton) {
			...
			if (addErrorPanel.txtEnfID.getText().length() > 0 && addErrorPanel.txtSapUserName.getText().length() > 0
					&& addErrorPanel.cmbDepartment.toString().length() > 0
					&& addErrorPanel.cmbShift.toString().length() > 0 && addErrorPanel.model.toString().length() > 0
					...
					&& addErrorPanel.cmbStatus.toString().length() > 0 && addErrorPanel.txtNotes.getText().length() > 0
					&& addErrorPanel.txtOpenedBy.getText().length() > 0) {
				DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MM/dd/yyyy");
				LocalDate ldt = LocalDate.now();
				DateFormat df = new SimpleDateFormat("MM/dd/yyyy");
				String opened_date = df.format(addErrorPanel.model.getValue());
				String dateOfError = df.format(addErrorPanel.errorDateModel.getValue());

				sql.createError(addErrorPanel.txtEnfID.getText(), addErrorPanel.txtSapUserName.getText(),
						addErrorPanel.cmbDepartment.getSelectedItem().toString(), 
						addErrorPanel.cmbShift.getSelectedItem().toString(),
						opened_date,
						addErrorPanel.txtProcess.getText(), addErrorPanel.txtSkuNumber.getText(),
						addErrorPanel.txtLocationAffected.getText(), addErrorPanel.txtQty.getText(), dateOfError,
						addErrorPanel.cmbStatus.getSelectedItem().toString(),
						(ldt.plusDays(7)).format(formatter).toString(), addErrorPanel.txtNotes.getText(),
						// attachment,
		...
		if (e.getSource() == addUserPanel.addUserSaveButton) {
			...
			if (addUserPanel.txtSapUserName.getText().length() > 0 && addUserPanel.txtAdminID.getText().length() > 0
					&& addUserPanel.txtJobTitle.getText().length() > 0 && addUserPanel.txtEmail.getText().length() > 0
				...
				sql.createUser(addUserPanel.txtAdminID.getText(), addUserPanel.txtSapUserName.getText(), myPass,
						(String) addUserPanel.cmbDepartment.getSelectedItem(), level,
						addUserPanel.txtJobTitle.getText(), addUserPanel.txtEmail.getText());
			} else {
				JOptionPane.showMessageDialog(frame, "Missing Information!", "Please fill in all information!",
						JOptionPane.INFORMATION_MESSAGE);
			}
		...
		if (e.getSource() == frame.adminEditErrorItem || e.getSource() == frame.devEditErrorItem
				|| e.getSource() == errorPanel.editButton) {
		...
		if (e.getSource() == frame.adminDeleteErrorItem || e.getSource() == frame.devDeleteErrorItem) {
			...
			// confirm delete
			int confirm = JOptionPane.showConfirmDialog(frame, "Are you sure you want to delete this error?",
			...
			if (confirm == JOptionPane.YES_OPTION) {
				...
				JOptionPane.showMessageDialog(frame, "Error Deleted!", "Deleted!", JOptionPane.INFORMATION_MESSAGE);
				...
				try {
					...
					// TODO Auto-generated catch block
					JOptionPane.showMessageDialog(frame, "Error refreshing data.", "An error has occurred,",
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (e.getSource() == console.runButton) {
						...
						try {
							...
							console.scrolls = new JScrollPane(console.outputTable,
							...
							console.model.sqlConsoleQuery(console.model, console.outputTable,
									console.inputArea.getText());
						} catch (Exception e4) {
							// TODO Auto-generated catch block
							e4.printStackTrace();
						}
		...
		try {
			...
			if (e.getSource() == login.loginButton) {
				...
				// if at least one is returned the password and username are
				// correct
				if (count > 0) {
					...
					JOptionPane.showMessageDialog(login, "Logged In!", "Logged In!", JOptionPane.INFORMATION_MESSAGE);

				} else {
					JOptionPane.showMessageDialog(login, "Wrong User Name/Password!", "Wrong Credentials",
							JOptionPane.ERROR_MESSAGE);
				}
				...
				if (password.length() > 0) {
					...
					if (pw.equals(password)) {
						...
						if (Arrays.equals(changePassword.confirmNewPassword.getPassword(),
							...
							JOptionPane.showMessageDialog(frame, "New password does not match, Please try again!",
									"New Password Mismatch", JOptionPane.ERROR_MESSAGE);
						}
					}

					// currentpassword is wrong
					else {
						JOptionPane.showMessageDialog(frame, "Current Password is Incorrect, Please try again!",
								"Wrong Password", JOptionPane.ERROR_MESSAGE);
					}
				}

				else {
					JOptionPane.showMessageDialog(frame, "Something went wrong when looking for the current password!",
		...
		if (e.getSource() == settingsPanel.settingsSaveButton) {
			settings.savePaths(settingsPanel.txtDatabasePath.getText(), settingsPanel.txtPdfPath.getText(),
					settingsPanel.txtPdfOutputPath.getText());
		...
		if (e.getSource() == settingsPanel.choosePDFTemplateLocation) {
			JFileChooser databaseFileChooser = new JFileChooser();
			databaseFileChooser.setCurrentDirectory(new java.io.File("."));
			databaseFileChooser.setDialogTitle("PDF Template Location");
			databaseFileChooser.setFileSelectionMode(databaseFileChooser.DIRECTORIES_ONLY);
			databaseFileChooser.setAcceptAllFileFilterUsed(false);

			// Get selected directory
			if (databaseFileChooser.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION) {
				File selectedFile = databaseFileChooser.getSelectedFile();
				settingsPanel.txtPdfPath.setText(databaseFileChooser.getSelectedFile().toString() + "\\");
			}
		}

		if (e.getSource() == settingsPanel.choosePDFOutputLocation) {
			JFileChooser databaseFileChooser = new JFileChooser();
			databaseFileChooser.setCurrentDirectory(new java.io.File("."));
			databaseFileChooser.setDialogTitle("PDF Output Location");
			databaseFileChooser.setFileSelectionMode(databaseFileChooser.DIRECTORIES_ONLY);
			databaseFileChooser.setAcceptAllFileFilterUsed(false);

			// Get selected directory
			if (databaseFileChooser.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION) {
				settingsPanel.txtPdfOutputPath.setText(databaseFileChooser.getSelectedFile().toString() + "\\");
			}
		}
		/*
		 * ================================= End Settings Panel
